...
export const getAddressString = (address: addressType): string => {
  return `${address?.street ? address?.street + ',' : 'Undisclosed Location,'} ${address?.city ? address?.city + ',' : ''} ${address?.state ? address?.state + ',' : ''} ${address?.zip ?? ''}`
}
...
export const removeUnderscoreAddSpace = (str: string) => str.replace(/_/gi, ' ')
...
export const interestPerMonCalculator = (
  ...
  parseInt(values.advanceOption.hoa) &&
    obj.data.push({
      ...
      percentage: `${amtPercentageCaluclator(totalInterestPerMonth, parseInt(values.advanceOption.hoa))}%`,
  ...
  parseInt(values.advanceOption.PMI) &&
    obj.data.push({
      ...
      percentage: `${amtPercentageCaluclator(totalInterestPerMonth, mortageInsurance)}%`,

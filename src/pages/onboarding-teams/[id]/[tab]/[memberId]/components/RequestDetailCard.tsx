...
export const RequestDetailCard = ({ data }: { data: any }) => {
  ...
  return (
    <Flex
      className={`${data?.status == 'pending' ? 'max-h-[520px]' : 'max-h-[590px]'} gap-[20px] max-w-[490px] px-[20px] py-[50px] border border-[#61687640]`}
      direction={'column'}
    >
      {detail?.map((i: any) => (
...
const RequestStat = ({ title, data }: { title: string; data: any }) => {
  return (
    <div className="grid grid-cols-2 gap-x-[20px]">
      ...
      <div
        className={`${title == 'Changes done by' ? 'col-span-2' : 'col-span-1'}`}
      >
        {title == 'Status' ? (
          <ColorTags
